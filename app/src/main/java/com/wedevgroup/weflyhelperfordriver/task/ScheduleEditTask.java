...
public  class ScheduleEditTask extends AsyncTask<Void, Integer, Boolean> {
    ...
    private String url = "";
    private OnScheduleEditListener listener;
    private View v;
    ...
    public ScheduleEditTask(@NonNull final CopyOnWriteArrayList<Parcelle> list, @NonNull final Context ctx){
            ...
            if (list.size() > 0){
                ...
                for (Parcelle pm: list){
                    ...
                    if (!response.trim().equals("") && !response.trim().equals(Constants.SERVER_ERROR) && !response.trim().equals(Constants.RESPONSE_EMPTY) && !response.trim().contains(Constants.RESPONSE_ERROR_HTML)){
                        ...
                        Calendar calendar = Utils.getDateFromStringJ(pm.getDateSurvol());
                        pm.setFlYear(calendar.get(Calendar.YEAR));
                        pm.setFlMonth(calendar.get(Calendar.MONTH) + 1);
                        pm.setFlDay(calendar.get(Calendar.DAY_OF_MONTH));


                        if (DataBasePresenter.getInstance().updateParcelle( pm)){
    ...
    protected void onPostExecute(Boolean isOk) {
        if(isCancelled())
            return;
        notifyOnScheduleEditListener(isOk, notifyList);
    }

    public void setOnScheduleEditListener(@NonNull OnScheduleEditListener listener, @NonNull View view) {
        this.listener = listener;
        this.v = view;
    }

    public static interface OnScheduleEditListener {
        void onEditError(@NonNull View view, boolean many);
        void onEditSucces(@NonNull final CopyOnWriteArrayList<Parcelle> newList, @NonNull View view);
    }

    private void notifyOnScheduleEditListener(boolean isDone, @NonNull final CopyOnWriteArrayList<Parcelle> mList) {
        if (listener != null  && v != null){
            try{
                if (isDone){
                    if (appController != null)
                        appController.resetNotificationDate();
                    listener.onEditSucces(mList, v);
                }
                else
                    listener.onEditError(v, false);
            }catch (Exception e){
                e.printStackTrace();
            }
        }
